...
- (void)setUpUI{
    ...
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CMScreenW, CMScreenH)];
...
- (void)displayAtPoint:(CGPoint)point{
    ...
    }else if (point.x >= CMScreenW - kMargin - kRadius - kTriangleHeight * 0.7){
    ...
    if (CGRectGetMaxY(self.frame) > CMScreenH) {
...
- (void)adjustPosition:(CGPoint)point{
    ...
    if (self.x < kMargin) {
        self.x = kMargin;
    }else if (self.x > CMScreenW - kMargin - self.width){
        self.x = CMScreenW - kMargin - self.width;
    }
    self.layer.affineTransform = CGAffineTransformMakeScale(1.0, 1.0);
}
